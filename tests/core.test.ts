import test from'node:test';import assert from'node:assert/strict';import{normalizeNumberSetting,scoreVotes}from'../src/game.ts';import{QUESTIONS,shuffle}from'../src/questions.ts';
...
test('number settings allow an empty draft and normalize on commit',()=>{assert.equal(normalizeNumberSetting('',1,65,5),5);assert.equal(normalizeNumberSetting('3',1,65,5),3);assert.equal(normalizeNumberSetting('999',1,65,5),65)});
